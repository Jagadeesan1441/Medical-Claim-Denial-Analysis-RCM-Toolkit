...
if uploaded_file:
    ...
    with tab2:
        ...
        model_path = os.path.join(os.getcwd(), 'deniednew.pkl')
        encoder_path = os.path.join(os.getcwd(), 'encodernew2.pkl')

        required_cols = ['CPT Code', 'Insurance Company', 'Physician', 'Amount']

        if not all(col in df.columns for col in required_cols):
            st.error(f"Missing required columns: {', '.join([col for col in required_cols if col not in df.columns])}")
            ...
            encoded = encoder.transform(input_data[['Insurance Company', 'Physician']])
            input_data[['Insurance Company', 'Physician']] = encoded
